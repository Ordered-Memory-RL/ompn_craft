...
RENDER_DIRID = {'right': 0, 'down': 3, 'left': 2, 'up': 1}
...
def load_icons(name, width, height):
    """ Load the icon at the given width and height """
    icon_name = os.path.join(RESOURCE_DIR, '{}.jpg'.format(name))
    img = Image.open(icon_name)
    img = img.resize((width, height), Image.BICUBIC).convert('RGB')
    return np.array(img)