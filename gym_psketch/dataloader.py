...
#def _create_batch(curr_t, batch_size, seq_len, long_trajs):
#    batch = []
#    for i in range(batch_size):
#        traj_seg = long_trajs[i][curr_t: curr_t + seq_len]
#        while len(traj_seg) < seq_len:
#            remaining = seq_len - len(traj_seg)
#            traj_seg += long_trajs[i][:remaining]
#        batch.append(traj_seg)
#    return _totensor(_stack(batch))
#
#
#def _stack(traj_list) -> DictList:
#    """ Stack them on batch dim """
#    res = DictList({})
...
#
#def _totensor(traj):
#    return DictList({key: torch.tensor(val) for key, val in traj.items()})
